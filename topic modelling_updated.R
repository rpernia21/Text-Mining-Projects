...
marcos <- marcos_raw %>%
  mutate(docvars = ifelse(str_detect(text, "GOVERNMENT"),
                        text,
                        NA)) %>%
  fill(docvars) %>%
  filter(docvars != "THE ADVENTURES OF SHERLOCK HOLMES") %>%
  mutate(docvars = factor(docvars, levels = unique(docvars)))
